...
export const DATA_UPDATER_PATH = '/internal/job/data-updater';

export const registerDataUpdaterJob: PathFactory = (router: Router) => {
    router.post(DATA_UPDATER_PATH, async (_req, _res): Promise<void> => {
        await runDataUpdater();
    });
};

export const forceDataUpdater = async () => {

    const logger = await Logger.Create('Force Data Updater');
...
export const runDataUpdater = async () => {
    const logger = await Logger.Create("Job - Data Updater");
    logger.traceStart(DATA_UPDATER_PATH);
        ...
        if (hasNewAtcfStorm)
        {
            ...
            const newAtcfStorm = newSummaryApiData.atcf.data
                .find(a => !lastSummaryApiData.atcf.data
                    .find(b => a.data[0] && b.data[0] && a.data[0].basin == b.data[0].basin && a.genNo == b.genNo));

            logger.info('New API result has a new storm in the ATCF. Reposting!');
            const result = await createSummaryPost(
                newAtcfStorm && newAtcfStorm.data[0]
                    ? `New ATCF Storm - ${newAtcfStorm.data[0].basin}${newAtcfStorm.data[0].stormNo}`
                    : 'New ATCF Storm',
                'New ATCF Storm',
                newAtcfStorm && newAtcfStorm.data[0]
                    ? `New ATCF Storm - ${newAtcfStorm?.data[0].basin}`
                    : 'New ATCF Storm'
            );
            logger.info('Created new post:', result.toast.text, result.post?.id);
            return;
